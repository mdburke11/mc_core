...
#include "mc/AnnealingRunner.hpp"
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        mc::AnnealingParams annealParams = mc::AnnealingParams::from(params);
        ...
        std::string runMode = params.getString("runMode", "pt");
        ...
        auto addSingleDerived = [&](auto& runner, double T) {
            double beta = 1.0 / T;
        ...
        auto addTemperatureDerived = [&](auto& runner) {
        ...
        if (runMode == "single") {
            ...
            addSingleDerived(runner, modelParams.T);

            runner.run();

        } else if (runMode == "anneal") {
            mc::AnnealingRunner<Ising2D, decltype(factory)>
                runner(factory, runParams, annealParams);

            addTemperatureDerived(runner);
            ...
            mc::ParallelTemperingRunnerCPU<Ising2D, decltype(factory)>
                runner(factory, runParams, scanParams, ptParams);
            ...
            throw std::runtime_error("Unknown runMode: " + runMode);
        }
